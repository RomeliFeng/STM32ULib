...
public:
	 ...
	 */
	T Get() {
		T delta = *_Input - _Last; //计算外循环一阶导数
		T delta2 = *_Input - _Last2; //计算内循环一阶导数
		T deltaError = delta - _LastDelta; //计算外循环二阶导数
		T deltaError2 = delta2 - _LastDelta2; //计算内循环二阶导数
		deltaError = deltaError < 0 ? -deltaError : deltaError; //正值化
		deltaError2 = deltaError2 < 0 ? -deltaError2 : deltaError2; //正值化
